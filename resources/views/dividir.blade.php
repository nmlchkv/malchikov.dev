...
(function () {
    ...
    function createPriceRow() {
        ...
        const addToPriceBtn = document.createElement('button');
        addToPriceBtn.type = 'button';
        addToPriceBtn.className = 'btn btn-outline-secondary btn-sm';
        addToPriceBtn.textContent = 'Add to price';

        row3.appendChild(peopleLabel);
        row3.appendChild(peopleSelect);
        row3.appendChild(addToPriceBtn);
        ...
        addToPriceBtn.addEventListener('click', () => {
        ...
        window.removePerson = function(personName) {
        ...
        amountInput.addEventListener('keydown', (e) => {
            ...
            if (e.key === '.') {
                if (amountInput.value.includes('.')) e.preventDefault();
                return;
            }
    ...
    function updatePeopleArray() {
        const inputs = Array.from(peopleList.querySelectorAll('input'));
        ...
        inputs.forEach(input => {
            ...
            if (firstByNorm.has(norm) && firstByNorm.get(norm) !== input) {
                input.classList.add('is-invalid');
                input.setCustomValidity('Duplicate name');
                input.title = 'Duplicate name';
            } else {
                input.classList.remove('is-invalid');
                input.setCustomValidity('');
                input.title = '';
                firstByNorm.set(norm, input);
            }
        });

        people = [];
        firstByNorm.forEach((input) => {
    ...
    function calculate() {
        ...
        // invalid amounts?
        if (pricesList.querySelector('.is-invalid')) {
            globalError.textContent = 'Please fix invalid amounts.';
        ...
        // duplicate names?
        if (peopleList.querySelector('.is-invalid')) {
            globalError.textContent = 'Please remove duplicate names.';
            globalError.classList.remove('d-none');
            results.classList.add('d-none');
            return;
        }
        ...
        prices.forEach(price => {
            ...
            } else if (price.selectedPeople.length > 0) {
        ...
        if (totalEntered > 0) {
            ...
            if (absDiff < 0.005) {
                ...
                comparison.textContent =
                    `Difference: ${absDiff.toFixed(2)} (${direction} the total). People sum: ${sumOfPeople.toFixed(2)}, Total: ${totalEntered.toFixed(2)}.`;
    ...
    // init
    for (let i = 0; i < 3; i++) peopleList.appendChild(createPersonRow());
    for (let i = 0; i < 2; i++) pricesList.appendChild(createPriceRow());
    updatePriceDropdowns();
})();
</script>
@endsection
